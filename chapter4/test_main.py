...
def test_read_players():
    response = client.get("/v0/players/?skip=0&limit=10_000")
    assert response.status_code == 200
...
def test_read_performances():
    response = client.get("/v0/performances/?skip=0&limit=20_000")
    assert response.status_code == 200
...
def test_read_performances_by_date():
    response = client.get(
        "/v0/performances/?skip=0&limit=20_000&minimum_last_changed_date=2024-04-01"
    )
    assert response.status_code == 200
